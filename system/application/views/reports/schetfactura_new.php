...
<FONT size=7>
 <table width=100%>
 <tr align=center>
 <td width=1850px> <b><?php echo (strlen(trim($akt_vypolnenyh_rabot))==0?"Есеп шот фактура / Счет-фактура":$akt_vypolnenyh_rabot);?> №</b> <?php 
...
if (strlen($data_schet)==0){ echo datetostring($schetfactura_date->date);} else { echo $data_schet;}?>
 ...
 <td width=2100px>Жеткізушінің ЖСК<br />ИИК поставщика <u>  <?php echo $org->IIK;?> КБЕ-<?php echo $org->Bank_kbe;?> КНП-<?php echo $org->bank_knp;?> <?php echo $org->bank_name;?><br />
			БИК <?php echo $org->bank_bik." ,". $org->svidetelstvo_nds;;?>  </u></td>              
 <td> </td>
 </tr>
 <tr>
 <td width=2100px>Тауарлар (жұмыстар, қызметтер) жеткізуге арналған шарт (келісім-шарт)<br/>Договор(контракт) на поставку товаров (работ, услуг) №<u>   <?php if (strlen($firm->dog2)==0) {echo $firm->dogovor."   </u> от <u>    ".datetostring($firm->dogovor_date);} else {echo $firm->dog2;}?></u></td>
 <td> </td>
 </tr>
 <tr>
 ...
 <tr>
 <td width=2100px>Сатып алушының БСН<br/>БИН и адрес нахождения покупателя <u> <?php echo "БИН ".$firm->bin.", ".$firm->address.", БИК ".$firm->bik;?></u></td>
 </tr>
 <tr>
...
<?php 
$sum_bez_nds=0;$sum_nds=0;$sum=0;$i=1;$sum_kvt=0;$sumnds=0;$sumitog=0;
...
<tr align="center">
	...
	<TD width="400px"><?php if (strlen($electric)==0) { echo "Электроэнергия за <br/>" . datetostring2($schetfactura_date->date) ."года";} else {echo $firm->electric; }?> </td>
	...
	<?php
		...
		{
			echo (f_d($s2->tenge*12/100));$sumnds+=(f_d($s2->tenge*12/100));
			$sum_nds+=$s2->nds_value;
			$i_nds-=f_d($s2->nds_value);
		}
		else
			{echo f_d3($posl*12/100);$sumnds=(f_d($posl*12/100))+$sumnds;}
	?>
	</td>
	...
	<?php 
		...
		{
			echo f_d($s2->tenge+$s2->tenge*12/100);$sumitog+=f_d($s2->tenge+$s2->tenge*12/100);
			$sum+=$s2->with_nds;
			$i_itogo-=f_d($s2->with_nds);
		}
		else
		{	echo f_d($posl+($posl*12/100));$sumitog=$sumitog+f_d($posl+($posl*12/100));}
	?>
	</td>
	<TD width="110px">0</td>
	<TD width="100px">0</TD>
