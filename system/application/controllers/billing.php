...
class Billing extends Controller
{
	...
	function schetfactura()
	{
		...
		if (isset($_POST['schetfact'])){
		$string=$this->load->view("reports/schetfactura_new_dog",$data,TRUE);
   ...
   if ($_POST['ture_id']!=-1) {	$sql_last.=" and ture_id=".$_POST['ture_id']; }
	...
	function nachislenie_v_buhgalteriu()
	{
		...
		set_time_limit(0);
		$db = dbase_open("c:/oplata/nach4.dbf", 2);
		...
		{			
			...
			$db2 = dbase_open("c:/oplata/nach4.dbf", 2);
			foreach ($nach->result() as $n)
			{
				dbase_add_record($db2,
					array (
 ...
 public function gos_poverka()
	...
	public function export_rekvizit_schet()
    {
        ...
        if ($db) {
            ...
            if (!copy("c:/oplata/rschet.dbf", "c:/oplata/akzhar{$month}.dbf")) {
    ...
    public function migration()
    {
        ...
        $this->export_to_excel("other_reports/migration", $data,"акжарский_рэс");
